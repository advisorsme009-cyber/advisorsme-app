...
import { AddToReportButton } from './report/reportShared';
...
// Map a selected metric → its deeper-level (Lv3) module when one exists in results.
const L3_MATCHERS = [
  { module: 'S&M', rx: /s\s*&\s*m|selling|marketing/i },
  { module: 'G&A', rx: /g\s*&\s*a|general\s*&?\s*admin|administ/i },
  { module: 'FA', rx: /fixed\s*asset|depreciat|capex|capital\s*expenditure/i },
  { module: 'WC', rx: /working\s*capital/i },
  { module: 'DEBT', rx: /debt|interest\s*expense|borrow|loan|financing\s*cost/i },
  { module: 'eosp', rx: /eosp|end[\s-]*of[\s-]*service|gratuit|severance/i },
  { module: 'equity', rx: /equity|share\s*capital|retained\s*earning/i },
];

function resolveDrillModule(key, label, results) {
  if (!results) return null;
  // Direct match when the metric key/label already names an Lv3 module
  for (const cat of LEVEL3_CATEGORIES) {
    if ((key === cat || (label && label.toLowerCase() === cat.toLowerCase())) && results[cat]) {
      return cat;
    }
  }
  const text = `${key || ''} ${label || ''}`;
  for (const m of L3_MATCHERS) {
    if (m.rx.test(text) && results[m.module]) return m.module;
  }
  return null;
}
...
function StackedSection({ title, mod, years, paletteKey, highlightYear, revenue, altMode, altLabel, clientId, reportMeta }) {
  ...
  // Pin the absolute (value) breakdown so it renders on a value axis in the Report Generator.
  const snapshot = useMemo(
    () => buildStacked(mod, years, palette, { mode: 'absolute', revenue }),
    [mod, years, palette, revenue]
  );
  const getPayload = () => ({
    ...(reportMeta || {}),
    title,
    series: {
      ...
      datasets: (snapshot?.datasets || []).map((d, i) => ({
        label: d.label,
        data: d.data,
        ...
        <Stack direction="row" spacing={1.5} alignItems="center">
          ...
          {reportMeta && (
            <AddToReportButton
              ...
              disabled={!snapshot || snapshot.datasets.length === 0}
            />
          )}
        </Stack>
      }
...
// Single-series analytics row (one line for the selected metric) with Chart/Table + Export
function MetricLineSection({ title, accent, labels, seriesLabel, data, isPercent = true, clientId, reportMeta }) {
  const [view, setView] = useState('chart');
  const chartRef = useRef(null);

  const hasData = data.some((v) => v !== null && v !== undefined && !isNaN(v));

  // Report Generator stores percent series as decimals (0.25 → 25%); this file keeps whole percents.
  const getPayload = () => ({
    ...(reportMeta || {}),
    title,
    series: {
      ...
      datasets: [
        {
          label: seriesLabel,
          data: data.map((v) =>
            v === null || v === undefined || isNaN(v) ? null : isPercent ? v / 100 : v
          ),
          color: accent,
        },
      ],
  ...
  const tableData = { labels, datasets: [{ label: seriesLabel, data }] };
  ...
  const options = {
    ...
    scales: {
      ...
      y: { grid: { color: '#F1F3F6' }, ticks: { font: { size: 11 }, callback: (v) => fmt(v) } },
    },
  };
  ...
  return (
    <ChartCard
      ...
      toggle={
        <Stack direction="row" spacing={1.5} alignItems="center">
          <ViewToggle value={view} onChange={setView} />
          <ExportMenu view={view} chartRef={chartRef} tableData={tableData} isPercent={isPercent} name={title} />
          {reportMeta && (
            <AddToReportButton clientId={clientId} getPayload={getPayload} disabled={!hasData} />
          )}
        </Stack>
      }
    >
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700, color: '#1E293B', fontSize: '0.8rem' }}>Series</TableCell>
                  {labels.map((y) => (
                    <TableCell key={y} align="right" sx={{ fontWeight: 700, color: '#475569', fontSize: '0.8rem' }}>
                      {y}
                    </TableCell>
                  ))}
                </TableRow>
              ...
              <TableBody>
                <TableRow>
                  <TableCell sx={{ fontWeight: 600, fontSize: '0.8rem', color: '#1E293B' }}>{seriesLabel}</TableCell>
                  {data.map((v, j) => (
                    <TableCell key={j} align="right" sx={{ fontSize: '0.8rem', fontVariantNumeric: 'tabular-nums' }}>
                      {fmt(v)}
                    </TableCell>
...
const ReportGeneratorLegacy = () => {
  ...
  // --- Primary selected metric → detailed breakdown + single-line analytics ---
  const primaryKey = selectedMetrics[0] || null;
  const primaryRaw = useMemo(
    () => (primaryKey ? currentData?.[primaryKey] || level3DataForCategory?.[primaryKey] || null : null),
    [primaryKey, currentData, level3DataForCategory]
  );
  const primaryLabel = primaryRaw?.param_name || primaryKey || '';

  // Deeper-level (Lv3) module this metric drills into, when one exists
  const drillModule = useMemo(
    () => (primaryKey ? resolveDrillModule(primaryKey, primaryLabel, results) : null),
    [primaryKey, primaryLabel, results]
  );

  // Module feeding the detailed breakdown: Lv3 components if available, else the metric itself as a single series
  const detailMod = useMemo(() => {
    if (drillModule) return prepareModuleData(results?.[drillModule]);
    if (primaryRaw && primaryKey) return { [primaryKey]: primaryRaw };
    return null;
  }, [drillModule, results, primaryRaw, primaryKey]);

  // Common-size base: revenue for the income statement, total assets for the balance sheet
  const totalAssetsItem = useMemo(
    () => findItem(bsMod, ['totalAssets', 'assetsTotal'], /total assets/i),
    [bsMod]
  );
  const commonSizeBase = statementType === 'BS' ? totalAssetsItem : revenueItem;

  // Single-line analytics for the primary metric (values stored as percentages)
  const pctOfRevData = useMemo(
    () =>
      years.map((y) => {
        const r = Number(revenueItem?.[y]) || 0;
        const v = Number(primaryRaw?.[y]);
        return r && isFinite(v) ? (v / r) * 100 : null;
      }),
    [years, revenueItem, primaryRaw]
  );
  const growthData = useMemo(
    () =>
      years.map((y, i) => {
        if (i === 0) return null;
        const cur = Number(primaryRaw?.[y]);
        const prev = Number(primaryRaw?.[years[i - 1]]);
  ...
  const commonSizeData = useMemo(
    () =>
      years.map((y) => {
        const base = Number(commonSizeBase?.[y]) || 0;
        const v = Number(primaryRaw?.[y]);
        return base && isFinite(v) ? (v / base) * 100 : null;
      }),
    [years, commonSizeBase, primaryRaw]
  );

  // --- Report Generator payloads for the two fixed top charts ---
  const heroIsPercent = showPctRevenue || showPctGrowth;
  const heroChartTypeMeta = showPctRevenue ? 'pctOfRevenue' : showPctGrowth ? 'growth' : 'value';
  const getHeroPayload = () => ({
    module: moduleKey,
    metric: selectedMetrics[0] || null,
    chartType: heroChartTypeMeta,
    title: 'Performance Trend',
    series: {
      labels: heroData?.labels || [],
      datasets: (heroData?.datasets || []).map((d) => ({
        label: d.label,
        data: (d.data || []).map((v) =>
          v === null || v === undefined || isNaN(v) ? null : heroIsPercent ? v / 100 : v
        ),
        color: typeof d.borderColor === 'string' ? d.borderColor : INK,
      })),
    },
  });

  // KPI Analysis is horizontal paired bars; the Report Generator renders it as percent lines.
  const getKpiPayload = () => ({
    module: 'IS-CON',
    metric: 'kpi',
    chartType: 'pctOfRevenue',
    title: 'KPI Analysis (%)',
    series: {
      labels: kpiBarData?.labels || [],
      datasets: (kpiBarData?.datasets || []).map((d) => ({
        label: d.label,
        data: (d.data || []).map((v) => (v === null || v === undefined || isNaN(v) ? null : v / 100)),
        color: typeof d.backgroundColor === 'string' ? d.backgroundColor : INK,
      })),
    },
  });
          ...
          <Grid container spacing={3}>
              ...
              <ChartCard
                ...
                toggle={
                  <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
                    ...
                    <AddToReportButton
                      clientId={clientId}
                      getPayload={getHeroPayload}
                      disabled={!heroData || heroData.datasets.length === 0}
                    />
                  </Stack>
                }
              >
            ...
            <Grid item xs={12} md={7}>
              <ChartCard
                ...
                toggle={
                  <Stack direction="row" spacing={1.5} alignItems="center">
                    <ExportButton chartRef={kpiRef} name="KPI Analysis" />
                    <AddToReportButton
                      clientId={clientId}
                      getPayload={getKpiPayload}
                      disabled={!kpiBarData}
                    />
                  </Stack>
                }
            ...
            {/* Detailed breakdown of the first selected metric (deeper Lv3 level when available) */}
            <Grid item xs={12}>
              {detailMod ? (
                <StackedSection
                  title={
                    drillModule
                      ? `${primaryLabel} — Detailed Breakdown (${drillModule})`
                      : `${primaryLabel} — Detail`
                  }
                  mod={detailMod}
                  years={years}
                  ...
                  altLabel="% of Revenue"
                  clientId={clientId}
                  reportMeta={{ module: drillModule || moduleKey, metric: primaryKey, chartType: 'drilldown' }}
                />
              ) : (
                <ChartCard title="Detailed Breakdown" accent={PALETTES.blue[5]} height={340}>
                  <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Typography color="text.secondary">
                      Select a metric above to see its detailed breakdown.
                    </Typography>
                  </Box>
                </ChartCard>
              )}
            ...
            {/* Three single-line analytics for the selected metric — one per row */}
            <Grid item xs={12}>
              <MetricLineSection
                title={`${primaryLabel || 'Metric'} — % of Revenue`}
                accent="#3B6FE0"
                labels={years}
                seriesLabel={primaryLabel || 'Metric'}
                data={pctOfRevData}
                clientId={clientId}
                reportMeta={{ module: moduleKey, metric: primaryKey, chartType: 'pctOfRevenue' }}
              />
            </Grid>
            <Grid item xs={12}>
              <MetricLineSection
                title={`${primaryLabel || 'Metric'} — % Growth`}
                accent="#3FC592"
                labels={years}
                seriesLabel={primaryLabel || 'Metric'}
                data={growthData}
                clientId={clientId}
                reportMeta={{ module: moduleKey, metric: primaryKey, chartType: 'growth' }}
              />
            </Grid>
            <Grid item xs={12}>
              <MetricLineSection
                title={`${primaryLabel || 'Metric'} — % Common Size`}
                accent="#9466E2"
                labels={years}
                seriesLabel={primaryLabel || 'Metric'}
                data={commonSizeData}
                clientId={clientId}
                reportMeta={{ module: moduleKey, metric: primaryKey, chartType: 'commonSize' }}
              />
            </Grid>
          </Grid>
        )}
...
export default ReportGeneratorLegacy;
